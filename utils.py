...
def performances(map_score_val_filename, map_score_test_filename):
    ...
    return val_threshold, best_test_threshold, val_ACC, val_ACER, val_APCER, val_BPCER, test_ACC, test_APCER, test_BPCER, test_ACER, test_threshold_ACER
...
# 1：真实人脸
# 0：攻击人脸
def temp_performances(map_score_test_filename):
    # test
    ...
    # test_labels: [1, 1, 1, 0, 1, 0, 1, 0...]
    # test_scores: [0.03, 0.06, 0.66, 0.07...]

    fpr, tpr, threshold = roc_curve(test_labels, test_scores, pos_label=1)
    test_err, test_threshold = get_err_threhold(fpr, tpr, threshold)

    # type1:FN  真实_正   预测_负
    type1 = len([s for s in data if s['map_score'] <= test_threshold and s['label'] == 1])
    # type2:FP  真实_负   预测_正
    type2 = len([s for s in data if s['map_score'] > test_threshold and s['label'] == 0])

    # type2:FP  真实_0   预测_1
    # type1:FN  真实_1   预测_0
    test_ACC = 1 - (type1 + type2) / count  # 1-所有预测错的/总数
    test_APCER = type2 / num_fake  # 测负实正/实负   就是说，在所有真实为负的样本中，预测成正的比例  越小越好
    test_BPCER = type1 / num_real  # 在所有真实为正的样本中，预测成负的比例  越小越好
    test_ACER = (test_APCER + test_BPCER) / 2.0  # 越小越好

    return test_ACC, test_APCER, test_BPCER, test_ACER, test_threshold